...
@DisplayName("SubscriptionUpdatedHandler Tests")
class SubscriptionUpdatedHandlerTest {

    private SubscriptionUpdatedHandler handler;
    ...
    @BeforeEach
    void setUp() {
        handler = new SubscriptionUpdatedHandler(subscriptionRepository, objectMapper, orchestrator);
    ...
    class Handle {
        ...
        @Test
        @DisplayName("should return SUBSCRIPTION_UPDATED event type")
        void should_return_correct_event_type() {
            assertEquals(StripeEventType.SUBSCRIPTION_UPDATED, handler.getEventType());
        }

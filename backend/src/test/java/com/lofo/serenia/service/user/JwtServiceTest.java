...
class JwtServiceTest {
    ...
    void should_generate_different_tokens_for_different_users() {
        ...
        UserResponseDTO user2 = new UserResponseDTO(UUID.randomUUID(), "Smith", "Jane", "other@example.com", "USER");
    ...
    void should_generate_token_for_admin_role() {
        UserResponseDTO adminUser = new UserResponseDTO(TEST_USER_ID, TEST_LAST_NAME, TEST_FIRST_NAME, TEST_EMAIL, "ADMIN");
        String token = jwtService.generateToken(adminUser);
        assertThat(token).isNotNull();
        assertThat(token).isNotEmpty();
    }
    @Test
    @DisplayName("should_include_correct_claims")
    void should_include_correct_claims() {
        UserResponseDTO user = createTestUser();
        String token = jwtService.generateToken(user);
        String[] parts = token.split("\\.");
        assertThat(parts).hasSize(3);
        String payload = new String(java.util.Base64.getUrlDecoder().decode(parts[1]));
        assertThat(payload).contains("\"upn\":\"" + TEST_EMAIL + "\"");
        assertThat(payload).contains("\"sub\":\"" + TEST_USER_ID + "\"");
        assertThat(payload).contains("\"groups\":[\"" + TEST_ROLE + "\"]");
    }
    @Test
    @DisplayName("should_set_correct_expiration")
    void should_set_correct_expiration() {
        UserResponseDTO user = createTestUser();
        String token = jwtService.generateToken(user);
        String[] parts = token.split("\\.");
        String payload = new String(java.util.Base64.getUrlDecoder().decode(parts[1]));
    ...
    private UserResponseDTO createTestUser() {
        return new UserResponseDTO(TEST_USER_ID, TEST_LAST_NAME, TEST_FIRST_NAME, TEST_EMAIL, TEST_ROLE);
    }
}
